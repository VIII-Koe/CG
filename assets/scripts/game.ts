...
export default class NewClass extends cc.Component {
    ...
    posCameraStart: cc.Vec3 = cc.v3(-310, 16, 0);

    countUpgrade: number = 0;
    ...
    startScene1() {
        ...
        cc.tween(this.container1).to(0.5, { position: cc.v3(-75, 125) }).start();
        cc.tween(this.container2).to(0.5, { position: cc.v3(-75, 125) }).delay(0.25).call(() => {
    ...
    buyCar1(event, customEventData) {
        ...
        this.carScene1.parent.getChildByName('car1').scale = 0.75;
        this.carScene1.parent.getChildByName('car2').scale = 0.75;
        this.carScene1.parent.getChildByName('car3').scale = 0.75;
        this.carScene2 = this.carScene2Icon[this.carIndex];
        this.carScene2.active = true;
        ...
        this.moveMoney(cc.v3(-68, 11, 0), targetPos, (this.priceCar / 500));
    }
        ...
        if (index === 2) {
            ...
            cc.tween(moneyNode).parallel(cc.tween().to(duration, { position: end }), scaleEffect).call(() => {
                ...
                if (this.carScene1.parent.active == true || this.step == 3) {
                    this.addMoney((this.step == 3) ? 2000000 : -100);
                    if (this.money == (8000 - this.priceCar) && this.carScene1.parent.active == true) {
    ...
    addPriceCar(price: number) {
        this.priceCar += price;
        if (this.priceCar > 100000000) {
            this.priceCar = 100000000;
        }
        this.carScene2.getChildByName('bid').getChildByName('moneylb').getComponent(cc.Label).string = this.priceCar.toString();
    }

    startScene2() {
        if (this.carIndex == 0) this.carIconList = this.carIconList1;
        else if (this.carIndex == 1) this.carIconList = this.carIconList2;
        else if (this.carIndex == 2) this.carIconList = this.carIconList3;
        this.carScene1.parent.active = false;
        this.carScene2.parent.active = true;
    }

    upgradeCar(event, customEventData) {
        ...
        this.carScene2.getChildByName('upgrade').getComponent(cc.Animation).play();
        btn.active = false;
        this.listBtn.children.forEach((child, index) => {
            if (index == this.countUpgrade) {
                child.active = false;
            } else {
                cc.tween(child).by(0.25, { y: 130 }).call(() => {
                    if (index == this.countUpgrade) {
                        child.children[1].getComponent(cc.Button).enabled = true;
                        child.getChildByName('hand').active = true;
                    }
                }).start();
            }
        });
        this.carIconList.forEach((child, index) => {
            if (index == this.countUpgrade + 1) {
                child.active = true;
            } else {
                child.active = false;
            }
    ...
    triggerUpgrade() {
        ...
        this.scheduleOnce(() => {
            ...
            cc.tween(car).set({ active: true, y: 750 }).to(0.25, { y: 350 }).call(() => {
